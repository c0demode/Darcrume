...
public class ManageChemsActivity extends AppCompatActivity {
    ...
    private void addNewChem(){
        if(okToAddChem){
            if(validateInput()){
                ...
                if(radBtnChemBw.isChecked())newChem.setBw_Color("BW");
                if(radBtnChemCol.isChecked())newChem.setBw_Color("Color");
                ...
                Toast.makeText(getApplicationContext(), "Fill all fields and try again", Toast.LENGTH_SHORT).show();
            }
        }else{
            Toast.makeText(getApplicationContext(), "Click Clear and try again", Toast.LENGTH_SHORT).show();
        }
